...
async function findFriendPath(startUsername, endUsername, progressCallback) {
    ...
    const MAX_PARALLEL_EXPAND = 7; // process 7 users at a time

    while (startQueue.length > 0 && endQueue.length > 0) {
        const expand = async (queue, visitedFromThisSide, visitedFromOtherSide) => {
            // Grab up to 7 users from the queue at once
            const batch = queue.splice(0, MAX_PARALLEL_EXPAND);

            // For progress display
            batch.forEach(([userId]) => {
                const username = userIdToName[userId];
                if (progressCallback) progressCallback(username);
                logCheckedUsers.push(username);
            });

            // Fetch friends for all batch users in parallel
            const friendsArrays = await Promise.all(batch.map(async ([userId]) => {
                return await getFriends(userId);
            }));

            // Process all friends returned
            for (let i = 0; i < batch.length; i++) {
                const [userId, path] = batch[i];
                const friends = friendsArrays[i];
...
app.get('/friend-path', async (req, res) => {
    ...
    res.write(`<!DOCTYPE html><html><head><title>Roblox Friend Path</title>
        ...
        <form method='get' action='/friend-path'>
            <input type='text' name='start' placeholder='Start Username' value='' required>
            <input type='text' name='end' placeholder='End Username' value='' required>
